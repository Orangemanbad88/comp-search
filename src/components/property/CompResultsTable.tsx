...
export function CompResultsTable({ results, onToggleSelect, onPropertyClick }: CompResultsTableProps) {
  ...
  const SortHeader = ({ label, sortKeyName, className }: { label: string; sortKeyName: SortKey; className?: string }) => (
    <th
      className={cn(
        'px-3 py-3 text-left text-xs font-semibold text-walnut dark:text-gold-light/70 uppercase tracking-wider cursor-pointer hover:text-burgundy dark:hover:text-gold select-none whitespace-nowrap transition-colors',
        className
      )}
  ...
  if (results.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="font-display text-lg font-medium text-charcoal dark:text-cream mb-1">No Comparables Found</h3>
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-walnut/10 dark:border-gold/10">
              ...
              <SortHeader label="Date" sortKeyName="saleDate" />
              <SortHeader label="Price" sortKeyName="salePrice" />
              <th className="px-3 py-3 text-left text-xs font-semibold text-walnut dark:text-gold-light/70 uppercase tracking-wider">
                Bed/Bath
              </th>
              <SortHeader label="Sq Ft" sortKeyName="sqft" />
              <SortHeader label="$/SF" sortKeyName="pricePerSqft" />
            ...
            {displayedResults.map((comp) => (
              ...
              >
                ...
                <td className="px-3 py-3">
                  {onPropertyClick && (
                    <button
                      onClick={(e) => { e.stopPropagation(); onPropertyClick(comp); }}
          ...
          >
            <div className="flex items-start justify-between gap-3 mb-3">
              ...
              <div className="text-right">
                <div className="font-display font-bold text-charcoal dark:text-cream">{formatCurrency(comp.salePrice)}</div>
                <div className="text-xs text-walnut/60 dark:text-cream/40">{formatDate(comp.saleDate)}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2 text-center text-sm border-t border-walnut/10 dark:border-gold/10 pt-3">
              ...
              <div>
                ...
                <div className="font-medium text-charcoal dark:text-cream">${comp.pricePerSqft}</div>
              </div>
              <div>
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-walnut/10 dark:border-gold/10">
              <div className="flex items-center gap-2">
                <span className="text-xs text-walnut/60 dark:text-cream/40">Match Score</span>
                <SimilarityBadge score={comp.similarityScore} />
              </div>
              {onPropertyClick && (
                <button

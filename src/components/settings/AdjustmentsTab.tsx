...
const FIELD_META: { key: keyof SaleAdjustmentValues; label: string; unit: string; step: number }[] = [
  { key: 'bedroom', label: 'Bedroom', unit: '$/bedroom', step: 1000 },
  { key: 'bathroom', label: 'Bathroom', unit: '$/bathroom', step: 1000 },
  { key: 'sqft', label: 'Square Footage', unit: '$/sqft', step: 10 },
  { key: 'age', label: 'Age/Year Built', unit: '$/year', step: 500 },
];

export const AdjustmentsTab = () => {
  const [profiles, setProfiles] = useState<AdjustmentProfile[]>([]);
  const [activeId, setActiveId] = useState<string | null>(null);
  const [values, setValues] = useState<SaleAdjustmentValues>(DEFAULT_SALE_ADJUSTMENTS);
  ...
  useEffect(() => {
    if (!activeId) {
      setValues(DEFAULT_SALE_ADJUSTMENTS);
      return;
    }
    const preset = SALE_ADJUSTMENT_PRESETS.find(p => p.id === activeId);
    if (preset) {
  ...
  const handleValueChange = (key: keyof SaleAdjustmentValues, raw: string) => {
          ...
          <div key={field.key}>
            ...
            <div className="relative">
              <input
                ...
                step={field.step}
                min={0}
                className="input-premium w-full px-3 py-2.5 pr-20 rounded-lg text-sm text-charcoal dark:text-cream"
              />
